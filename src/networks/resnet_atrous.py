...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, atrous=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes) #SynchronizedBatchNorm2d(planes, momentum=bn_mom)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        #self.bn2 = nn.BatchNorm2d(planes)
        self.bn2 = nn.BatchNorm2d(planes) #SynchronizedBatchNorm2d(planes, momentum=bn_mom)
...
#                          nn.Conv2d(64,64,kernel_size=3, stride=1, padding=1),
#                      )
        self.bn1 = nn.BatchNorm2d(64) #SynchronizedBatchNorm2d(64, momentum=bn_mom)
        self.relu = nn.ReLU(inplace=True)
...
def resnet50_atrous(pretrained=True, os=16, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet50'], model_dir='.')
...
def resnet101_atrous(pretrained=True, os=16, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet101'], model_dir='.')
        model_dict = model.state_dict()
        old_dict = {k: v for k,v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict) 
...
def resnet152_atrous(pretrained=True, os=16, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet152'], model_dir='.')
        model_dict = model.state_dict()
        old_dict = {k: v for k,v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict) 
