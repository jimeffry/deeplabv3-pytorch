...
class SCAR(nn.Module):
    def __init__(self, load_weights=False):
        ...
        self.output_layer = SCAModule(64, 22)
...
class SCAModule(nn.Module):
    def __init__(self, inn, out):
        ...
        self.conv_cat = Conv2d(base*2, out, 1, NL=None,same_padding=True, bn=False)
        self.initialize_weights()
        ...
        for idx, m in enumerate(self.modules()):
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
                if idx==31:
                    nn.init.constant_(m.bias,1/22.0)
            elif isinstance(m, nn.BatchNorm2d):
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)
