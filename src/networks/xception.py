...
class SeparableConv2d(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size=1,stride=1,padding=0,dilation=1,bias=False,activate_first=True,inplace=True):
        ...
        self.bn1 = nn.BatchNorm2d(in_channels) #SynchronizedBatchNorm2d(in_channels, momentum=bn_mom)
        self.relu1 = nn.ReLU(inplace=True)
        self.pointwise = nn.Conv2d(in_channels,out_channels,1,1,0,1,1,bias=bias)
        self.bn2 = nn.BatchNorm2d(out_channels) #SynchronizedBatchNorm2d(out_channels, momentum=bn_mom)
...
class Block(nn.Module):
    def __init__(self,in_filters,out_filters,strides=1,atrous=None,grow_first=True,activate_first=True,inplace=True):
        ...
        if out_filters != in_filters or strides!=1:
            ...
            self.skipbn =  nn.BatchNorm2d(out_filters) #SynchronizedBatchNorm2d(out_filters, momentum=bn_mom)
...
class Xception(nn.Module):
    ...
    def __init__(self, os):
        ...
        self.bn1 = nn.BatchNorm2d(32) #SynchronizedBatchNorm2d(32, momentum=bn_mom)
        self.relu = nn.ReLU(inplace=True)
        
        self.conv2 = nn.Conv2d(32,64,3,1,1,bias=False)
        self.bn2 = nn.BatchNorm2d(64) #SynchronizedBatchNorm2d(64, momentum=bn_mom)
        #do relu here
...
def xception(pretrained=True, os=16):
    ...
    if pretrained:
        old_dict = torch.load(model_urls['xception'],model_dir='.')
