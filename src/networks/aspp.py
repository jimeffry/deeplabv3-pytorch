...
class ASPP(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
        ...
        self.branch2 = nn.Sequential(
                nn.Conv2d(dim_in, dim_out, 3, 1, padding=6, dilation=6,bias=True),
                # SynchronizedBatchNorm2d(dim_out, momentum=bn_mom),
                nn.BatchNorm2d(dim_out),
                nn.ReLU(inplace=True),    
        )
        self.branch3 = nn.Sequential(
                nn.Conv2d(dim_in, dim_out, 3, 1, padding=12, dilation=12,bias=True),
                # SynchronizedBatchNorm2d(dim_out, momentum=bn_mom),
                nn.BatchNorm2d(dim_out),
                nn.ReLU(inplace=True),    
        )
        self.branch4 = nn.Sequential(
                nn.Conv2d(dim_in, dim_out, 3, 1, padding=18, dilation=18,bias=True),
                # SynchronizedBatchNorm2d(dim_out, momentum=bn_mom),
                nn.BatchNorm2d(dim_out),
                nn.ReLU(inplace=True),    
        )
...
#        )
    def forward(self, x):
        ...
        global_feature = F.interpolate(global_feature, (row,col), None, 'nearest')
